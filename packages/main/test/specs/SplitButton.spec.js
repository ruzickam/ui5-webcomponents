...
describe("Split Button general interaction", () => {
	...
	it("tests inner text button icon", async () => {
		await browser.url(`http://localhost:${PORT}/test-resources/pages/SplitButton.html`);
		const sbTextIcon = await browser.$("#sbDefault");
		const icon = await sbTextIcon.getAttribute("icon");
		const textButton = await sbTextIcon.shadow$(".ui5-split-text-button");

		assert.strictEqual(await textButton.getAttribute("icon"), icon, "Text button have proper icon");
	});
